...
bool LINUX_VL53L1X::begin() { return _device->VL53L1X_SensorInit(); }

/*Checks the ID of the device, returns true if ID is correct*/

bool LINUX_VL53L1X::checkID() {
  uint16_t sensorId;
...
VL53L1X_Version_t LINUX_VL53L1X::getSoftwareVersion() {
  VL53L1X_Version_t tempVersion;
  _device->VL53L1X_GetSWVersion(&tempVersion);
  return tempVersion;
}

void LINUX_VL53L1X::setI2CAddress(uint8_t addr) {
  _i2cAddress = addr;
  _device->VL53L1X_SetI2CAddress(addr);
}

int LINUX_VL53L1X::getI2CAddress() { return _i2cAddress; }
...
uint8_t LINUX_VL53L1X::getInterruptPolarity() {
  uint8_t tmp;
  _device->VL53L1X_GetInterruptPolarity(&tmp);
  return tmp;
}

void LINUX_VL53L1X::startRanging() { _device->VL53L1X_StartRanging(); }

void LINUX_VL53L1X::stopRanging() { _device->VL53L1X_StopRanging(); }

bool LINUX_VL53L1X::checkForDataReady() {
  uint8_t dataReady;
...
bool LINUX_VL53L1X::checkBootState() {
  uint8_t bootState;
  _device->VL53L1X_BootState(&bootState);
  return (bool)bootState;
}

uint16_t LINUX_VL53L1X::getSensorID() {
  uint16_t id;
  _device->VL53L1X_GetSensorId(&id);
  return id;
}

uint16_t LINUX_VL53L1X::getDistance() {
  uint16_t distance;
  _device->VL53L1X_GetDistance(&distance);
  return (int)distance;
}

uint16_t LINUX_VL53L1X::getSignalPerSpad() {
  uint16_t temp;
  _device->VL53L1X_GetSignalPerSpad(&temp);
  return temp;
}

uint16_t LINUX_VL53L1X::getAmbientPerSpad() {
  uint16_t temp;
  _device->VL53L1X_GetAmbientPerSpad(&temp);
  return temp;
}

uint16_t LINUX_VL53L1X::getSignalRate() {
  uint16_t temp;
  _device->VL53L1X_GetSignalRate(&temp);
  return temp;
}

uint16_t LINUX_VL53L1X::getSpadNb() {
  uint16_t temp;
  _device->VL53L1X_GetSpadNb(&temp);
  return temp;
}

uint16_t LINUX_VL53L1X::getAmbientRate() {
  uint16_t temp;
  _device->VL53L1X_GetAmbientRate(&temp);
  return temp;
}

uint8_t LINUX_VL53L1X::getRangeStatus() {
  uint8_t temp;
...
uint16_t LINUX_VL53L1X::getDistanceThresholdLow() {
  uint16_t temp;
  _device->VL53L1X_GetDistanceThresholdLow(&temp);
  return temp;
}

uint16_t LINUX_VL53L1X::getDistanceThresholdHigh() {
  uint16_t temp;
...
uint16_t LINUX_VL53L1X::getROIY() {
  uint16_t tempX;
  uint16_t tempY;
  _device->VL53L1X_GetROI_XY(&tempX, &tempY);
  return tempY;
