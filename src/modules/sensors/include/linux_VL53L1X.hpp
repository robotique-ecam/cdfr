...
class LINUX_VL53L1X {
public:
  LINUX_VL53L1X(I2C &i2cPort = I2C(1), int shutdownPin = -1,
  ...
  VL53L1X_Version_t
  getSoftwareVersion();             // Get's the current ST software version
  ...
  void
  setInterruptPolarityHigh(); // Set the polarity of an active interrupt to High
  void
  setInterruptPolarityLow(); // Set the polarity of an active interrupt to Low
  ...
  void setTimingBudgetInMs(
      uint16_t timingBudget);     // Set the timing budget for a measurement
  ...
  uint8_t getDistanceMode(); // Get the distance mode, returns 1 for short and 2
                             // for long
  void setIntermeasurementPeriod(
      uint16_t intermeasurement);       // Set time between measurements in ms
  ...
  uint16_t getSignalPerSpad();  // Returns the average signal rate per SPAD (The
                                // sensitive pads that detect light, the VL53L1X
                                // has a 16x16 array of these) in kcps/SPAD, or
                                // kilo counts per second per SPAD.
  uint16_t getAmbientPerSpad(); // Returns the ambient noise when not measuring
                                // a signal in kcps/SPAD.
  uint16_t
  getSignalRate();      // Returns the signal rate in kcps. All SPADs combined.
  uint16_t getSpadNb(); // Returns the current number of enabled SPADs
  uint16_t getAmbientRate(); // Returns the total ambinet rate in kcps. All
                             // SPADs combined.
  uint8_t getRangeStatus(); // Returns the range status, which can be any of the
                            // following. 0 = no error, 1 = signal fail, 2 =
                            // sigma fail, 7 = wrapped target fail
  void setOffset(int16_t offset); // Manually set an offset in mm
  int16_t getOffset();            // Get the current offset in mm
  void
  setXTalk(uint16_t xTalk); // Manually set the value of crosstalk in counts per
                            // second (cps), which is interference from any sort
  ...
  void setDistanceThreshold(
      ...
      uint8_t window); // Set bounds for the interrupt. lowThresh and hiThresh
                       // are the bounds of your interrupt while window decides
                       // when the interrupt should fire. The options for window
                       // are shown below.
  ...
  uint16_t
  getDistanceThresholdWindow(); // Returns distance threshold window option
  ...
  * To set the center, set the pad that is to the right and above the exact
  ...
  void setROI(
      uint8_t x, uint8_t y,
      uint8_t opticalCenter); // Set the height and width of the ROI(region
                              // of interest) in SPADs, lowest possible option
                              // is 4. Set optical center based on above table
  ...
  void setSignalThreshold(
      uint16_t signalThreshold); // Programs the necessary threshold to trigger
                                 // a measurement. Default is 1024 kcps.
  uint16_t getSignalThreshold(); // Returns the signal threshold in kcps
  void
  setSigmaThreshold(uint16_t sigmaThreshold); // Programs a new sigma threshold
                                              // in mm. (default=15 mm)
  uint16_t getSigmaThreshold();  // Returns the current sigma threshold.
  void startTemperatureUpdate(); // Recalibrates the sensor for temperature
                                 // changes. Run this any time the temperature
                                 // has changed by more than 8°C
  void calibrateOffset(
      uint16_t
          targetDistanceInMm); // Autocalibrate the offset by placing a target a
                               // known distance away from the sensor and
                               // passing this known distance into the function.
  void calibrateXTalk(
      uint16_t targetDistanceInMm); // Autocalibrate the crosstalk by placing a
                                    // target a known distance away from the
