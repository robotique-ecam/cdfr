...
body_colors = {
    "red": {
        "bounds": [([0, 110, 110], [10, 255, 255]), ([170, 50, 50], [180, 255, 255])],
        "color": (0, 0, 200),
    },
    "green": {
        "bounds": [([50, 0, 50], [255, 10, 255]), ([35, 50, 35], [255, 255, 255])],
        "color": (0, 250, 0),
    },
}
...
class OnBoardService(Node):
    def __init__(self):
        ...
        self.srv = self.create_service(
            Trigger, "/get_eceuil_case", self.get_eceuil_case_callback
        )
        self.srv = self.create_service(
            Trigger, "/get_north_or_south", self.get_north_or_south_callback
        )
    ...
    def accepted(self, rectangle):
        ...
        for i in rectangle:
            ...
            if (
                (dist <= dist1 * (1 + accuracy / 2))
                and (dist >= dist1 * (1 - accuracy / 2))
            ) or (
                (dist1 <= dist * (1 + accuracy / 2))
                and (dist1 >= dist * (1 - accuracy / 2))
            ):  # and dist1 < dist:
                trueRect.append(i)
        return trueRect
