...
class StrategixActionServer(Node):
    ...
    def available_callback(self, request, response):
        ...
        if self.side.value == 'blue':
            response.available = [action for action in actions if not actions[action].get("ONLY_YELLOW") and not actions[action].get("STATUS")]
        else:
            response.available = [action for action in actions if not actions[action].get("ONLY_BLUE") and not actions[action].get("STATUS")]
