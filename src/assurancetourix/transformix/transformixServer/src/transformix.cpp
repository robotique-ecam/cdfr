...
Transformix::Transformix() : Node("Transformix") {
  ...
  service_all = this->create_service<transformix_services::srv::TransformixParameters>("transform_poseStamped",
              &generate_buffer_and_transform_my_pose_stamped);

  service_transform = this->create_service<transformix_services::srv::TransformixParametersTransformStamped>("get_transform",
          &get_transform_stamped_from_frames);

  service_pose = this->create_service<transformix_services::srv::TransformixParametersTransfromPose>("get_poseStamped_from_transform",
              &get_poseStamped_from_transformStamped);
...
void Transformix::generate_buffer_and_transform_my_pose_stamped(
          const std::shared_ptr<transformix_services::srv::TransformixParameters::Request> request,
          std::shared_ptr<transformix_services::srv::TransformixParameters::Response> response){
  ...
  RCLCPP_INFO(rclcpp::get_logger("transformix"),
          "Time given to buffer initialisation: %f", time_taken_total);
...
void Transformix::get_transform_stamped_from_frames(
          const std::shared_ptr<transformix_services::srv::TransformixParametersTransformStamped::Request> request,
          std::shared_ptr<transformix_services::srv::TransformixParametersTransformStamped::Response> response){
...
void Transformix::get_poseStamped_from_transformStamped(
          const std::shared_ptr<transformix_services::srv::TransformixParametersTransfromPose::Request> request,
          std::shared_ptr<transformix_services::srv::TransformixParametersTransfromPose::Response> response){
...
void Transformix::get_transform(tf2_ros::Buffer& tfBuffer, std::string& from_frame, std::string& to_frame, geometry_msgs::msg::TransformStamped& transformStampedOut) {
  ...
  try{
    transformStampedOut = tfBuffer.lookupTransform(to_frame, from_frame,
...
bool Transformix::canTransform(tf2_ros::Buffer& tfBuffer, std::string& from_frame, std::string& to_frame){
  ...
  try{
    ...
    if (transformation_possible) {RCLCPP_INFO(rclcpp::get_logger("transformix"), "True");}
    else{RCLCPP_INFO(rclcpp::get_logger("transformix"), "false");}
...
void Transformix::setTransform(geometry_msgs::msg::TransformStamped& transform, std::string frame_id, std::string child_frame_id, double posx, double posy, double posz, double rotw, double rotx, double roty, double rotz){
...
Transformix::~Transformix() {
  RCLCPP_INFO(rclcpp::get_logger("transformix"), "Transformix node terminated");
}
